...
exports.create = (req, res) => {
  ...
  evento
    ...
    .catch(err => {
      res.status(500).send({
        message:
          err.message || "Ha ocurrido un error mientras intentaba crear wl evento."
      });
    });
};
...
exports.findOne = (req, res) => {
  ...
  Evento.findOne(condition)
    .then(data => {
      if (!data)
        res.status(404).send({ message: "No se ha encontrado un evento con el nombre" + nombre });
      else res.send(data);
    })
    .catch(err => {
      res
        .status(500)
        .send({ message: "Error al recuperar el evento con el nombre= " + nombre });
    });
};
...
exports.buscaPorTipos = (req, res) => {
  ...
  Evento.find(condition).populate('tipoActividad')
  .then(data => {
    if (!data)
      res.status(404).send({ message: "No se ha encontrado un evento con el tipo" + id });
    else res.send(data);
  })
  .catch(err => {
    res
      .status(500)
      .send({ message: "Error al recuperar los eventos"});
  });
};
...
exports.buscaPorLugar = (req, res) => {
  ...
  Evento.find(condition).populate('lugar')
  .then(data => {
    if (!data)
      res.status(404).send({ message: "No se ha encontrado un evento en el lugar con id= " + id });
    else res.send(data);
  })
  .catch(err => {
    res
      .status(500)
      .send({ message: "Error al recuperar el evento" });
  });
};
...
exports.deleteAll = (req, res) => {
    ...
    .catch(err => {
      res.status(500).send({
        message:
          err.message || "Ocurrió un error al eliminar los eventos."
      });
    });
};
