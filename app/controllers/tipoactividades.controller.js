...
exports.create = (req, res) => {
  ...
  tipoActividad
    ...
    .catch(err => {
      res.status(500).send({
        message:
          err.message || "Ha ocurrido un error mientras intentaba crear el tipo de actividad."
      });
    });
};
...
exports.findAll = (req, res) => {
  ...
  TipoActividad.find(condition)
    ...
    .catch(err => {
      res.status(500).send({
        message:
          err.message || "Se ha producido un error al recuperar los tipos de actividad."
      });
    });
};
...
exports.findOne = (req, res) => {
  ...
  TipoActividad.findOne(condition)
    .then(data => {
      if (!data)
        res.status(404).send({ message: "No se ha encontrado un tipo con el nombre" + nombre });
      else res.send(data);
    })
    .catch(err => {
      res
        .status(500)
        .send({ message: "Error al recuperar el tipo de lugar con el nombre= " + nombre });
    });
};
...
exports.update = (req, res) => {
  ...
  TipoActividad.updateOne(condition, req.body, { useFindAndModify: false })
    .then(data => {
      ...
      } else res.send({ message: "El tipoActividad se ha actualizado con éxito." });
    })
    .catch(err => {
      res.status(500).send({
