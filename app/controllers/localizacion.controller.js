...
exports.findAll = (req, res) => {
  ...
  Localizacion.find(condition)
    ...
    .catch(err => {
      res.status(500).send({
        message:
          err.message || "Se ha producido un error al recuperar las localizaciones."
      });
    });
};
...
exports.findOne = (req, res) => {
  ...
  Localizacion.findOne(condition)
    .then(data => {
      if (!data)
        res.status(404).send({ message: "No se ha encontrado una localizacion con el nombre" + nombre });
      else res.send(data);
    })
    .catch(err => {
      res
        .status(500)
        .send({ message: "Error al recuperar la localizacion con el nombre= " + nombre });
    });
};
...
exports.update = (req, res) => {
  ...
  Localizacion.updateOne(condition, req.body, { useFindAndModify: false })
    .then(data => {
      ...
      } else res.send({ message: "La localizacion se ha actualizado con éxito." });
    })
    .catch(err => {
      res.status(500).send({
