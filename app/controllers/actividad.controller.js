...
exports.findOne = (req, res) => {
    ...
    Actividad.findOne(condition)
      .then(data => {
        if (!data)
          res.status(404).send({ message: "No se ha encontrado una actividad con el nombre" + nombre });
        else res.send(data);
      })
      .catch(err => {
        res
          .status(500)
          .send({ message: "Error al recuperar la actividad con nombre= " + nombre });
      });
};
...
exports.buscaPorTipos = (req, res) => {
  ...
  Actividad.find(condition).populate('tipoActividad')
  .then(data => {
    if (!data)
      res.status(404).send({ message: "No se ha encontrado una actividad con el tipo" + id });
    else res.send(data);
  })
  .catch(err => {
    res
      .status(500)
      .send({ message: "Error al recuperar la actividad"});
  });
};
...
exports.buscaPorLugar = (req, res) => {
  ...
  Actividad.find(condition).populate('lugar')
  .then(data => {
    if (!data)
      res.status(404).send({ message: "No se ha encontrado una actividad en el lugar con id= " + id });
    else res.send(data);
  })
  .catch(err => {
    res
