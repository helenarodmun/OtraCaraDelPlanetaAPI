...
exports.findOne = (req, res) => {
  ...
  Lugar.findOne(condition)
    .then(data => {
      if (!data)
        res.status(404).send({ message: "No se ha encontrado un lugar con el nombre" + nombre });
      else res.send(data);
    })
    .catch(err => {
      res
        .status(500)
        .send({ message: "Error al recuperar al lugar con el nombre= " + nombre });
    });
};
...
exports.buscaPorlocalizacion = (req, res) => {
  ...
  Lugar.find(condition).populate('localizacion')
  .then(data => {
    if (!data)
      res.status(404).send({ message: "No se ha encontrado un lugar en la localización  con id= " + id });
    else res.send(data);
  })
  .catch(err => {
    res
...
exports.buscaPorTipos = (req, res) => {
  ...
  .then(data => {
    if (!data)
      res.status(404).send({ message: "No se ha encontrado un lugar con el tipo con id= " + id });
    else res.send(data);
  })
  .catch(err => {
    res
...
exports.update = (req, res) => {
  ...
  Lugar.updateOne(condition, req.body, { useFindAndModify: false })
    .then(data => {
      ...
      } else res.send({ message: `El lugar ${nombre} se ha actualizado con éxito.` });
    })
    .catch(err => {
      res.status(500).send({
...
exports.deleteAll = (req, res) => {
    ...
    .catch(err => {
      res.status(500).send({
        message:
          err.message || "Ocurrió un error al eliminar los lugares."
      });
    });
};
